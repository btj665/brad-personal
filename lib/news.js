...
import { leanBucket, balanceCounts } from './lean.js';
...
// Hard-news feeds only — top-level news sections, no opinion pages. The roster
// deliberately spans left / center / right rated outlets so no political lean
// dominates the default mix (see lib/lean.js); any outlet can be toggled off
...
const SECTION_FEEDS = {
  world: [
    ...
    { name: 'UPI', url: 'https://rss.upi.com/news/world_news.rss' },
    { name: 'Washington Times', url: 'https://www.washingtontimes.com/rss/headlines/news/world/' },
    { name: 'Christian Science Monitor', url: 'https://rss.csmonitor.com/feeds/world' },
  ],
  us: [
    ...
    { name: 'UPI', url: 'https://rss.upi.com/news/us_news.rss' },
    { name: 'Washington Times', url: 'https://www.washingtontimes.com/rss/headlines/news/national/' },
    { name: 'The Hill', url: 'https://thehill.com/feed/' },
    { name: 'Christian Science Monitor', url: 'https://rss.csmonitor.com/feeds/usa' },
  ],
};
...
export function clusterItems(items) {
  ...
  return clusters
    .map((c) => {
      // Freshness factor: a story breaking now counts ~4x a two-day-old one,
      // so breaking news balloons quickly even before wide coverage.
      const ageHours = c.latest ? Math.max(0, (Date.now() - c.latest) / 3600e3) : 48;
      const freshness = 0.5 + 1.5 * Math.exp(-ageHours / 18);
      // Headline wording carries spin: when a center-rated outlet covers the
      // story, use its (most recent) headline as the cluster title.
      const centerItem = c.items.find((it) => leanBucket(it.sourceName) === 'center');
      return {
      id: c.id,
      title: centerItem ? centerItem.title : c.title,
      balance: balanceCounts(c.items),
      image: c.image,
      latest: c.latest,
      sourceCount: c.sources.size,
      sources: [...c.sources].filter((s) => s !== 'unknown'),
      weight: Math.round((c.sources.size * 2 + c.items.length) * freshness * 100) / 100,
...
export async function getSection(section, location, { fresh = false, exclude = new Set() } = {}) {
  ...
  const sources = [...counts.entries()]
    .map(([name, count]) => ({
      name, count, enabled: !exclude.has(name), lean: leanBucket(name),
    }))
