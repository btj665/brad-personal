...
export function clusterItems(items) {
  ...
  return clusters
    .map((c) => ({
      ...
      items: c.items.map(({ title, link, description, image, sourceName, timestamp }) => ({
        title, link, description, image, sourceName, timestamp,
      })),
    }))
...
export async function getSection(section, location, { fresh = false } = {}) {
  let feeds;
  ...
  return {
    ...
    fetchedAt: Date.now(),
    clusters: clusterItems(filtered).slice(0, 40),
    errors,
  };
}
